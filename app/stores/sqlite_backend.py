"""SQLite + numpy storage backend (default).

SQLite for rows + brute-force numpy cosine for vector similarity. No account, no
network, no extra service — plenty fast for prototype volumes. All stored vectors
...
def all_query_logs() -> list[dict]:
    conn = get_conn()
    rows = conn.execute("SELECT id, text, embedding FROM query_logs ORDER BY id").fetchall()
    return [{"id": r["id"], "text": r["text"], "vec": _from_blob(r["embedding"])} for r in rows]


def set_query_cluster(query_id: int, cluster_id: int) -> None:
    conn = get_conn()
    conn.execute("UPDATE query_logs SET cluster_id = ? WHERE id = ?", (cluster_id, query_id))
    conn.commit()
...
def recent_count_for_cluster(cluster_id: int, window_days: int) -> int:
    ...
    if window_days and window_days > 0:
        ...
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM query_logs WHERE cluster_id = ?", (cluster_id,)
        ).fetchone()
    return int(row["c"])
...
def feedback_stats() -> dict:
    ...
    row = conn.execute(
        ...
        "COALESCE(SUM(CASE WHEN vote > 0 THEN 1 ELSE 0 END), 0) AS up FROM feedback"
    ).fetchone()
    total = int(row["total"])
    return {"count": total, "helpful_rate": (int(row["up"]) / total) if total else None}
...
def cache_lookup(vec: np.ndarray, threshold: float) -> dict | None:
    ...
    if float(sims[best]) >= threshold:
        return {"answer": rows[best]["answer"], "citations": json.loads(rows[best]["citations"])}
    return None
...
def all_clusters() -> list[dict]:
    conn = get_conn()
    return [_cluster_row_to_dict(r) for r in conn.execute("SELECT * FROM clusters").fetchall()]
...
def replace_clusters(new_clusters: list[dict]) -> dict[int, int]:
    ...
    for i, c in enumerate(new_clusters):
        cur = conn.execute(
            ...
            (_to_blob(_normalize(c["centroid"])), int(c["member_count"]),
             json.dumps(c["representative_queries"]), now, now),
        )
